...
def main():
    points = []
    n=0
    with open("Dataset for R-Tree.txt", 'r') as dataset: #https://www.w3schools.com/python/ref_file_readlines.asp
    ...
    queries =[]
    with open("100 Queries.txt", 'r') as query:
        for d in query.readlines():
            d = d.split()
            queries.append({
                'x1': int(d[0]),
                'x2': int(d[1]),
    ...
    Rtree_start_time=time.time() #https://www.programiz.com/python-programming/time
    ...
    Rtree_end_time=time.time() #https://www.programiz.com/python-programming/time
    Timecost=Rtree_end_time-Rtree_start_time #calculate the total time to build R-Tree
    print("R-Tree construction completed.")
    print("The time-cost of building up the R-Tree is",Timecost,"seconds.")
    
    #Display query time using R-Tree
    results = []
    Rtree_query_start=time.time() #start time to query data using r-tree
    for query in queries:
        results.append(rtree.query(rtree.root, query))
    #Create output to save data points in the query
    with open("output.txt","w") as f:
        for i in results:
            f.write(str(i)+ '\n')

    Rtree_query_end=time.time() #end time to query data using r-tree
    Rtree_time=Rtree_query_end-Rtree_query_start #query processing time calculate by end time minus start time
    Average_rtree_time = Rtree_time/100 #calculate average time display each query by total processing time seperate total datapoint
    
    print ("There are",n,"queries.\n")
    print ("The R-Tree query time is",Rtree_time,"seconds.")
    print("The average time of R-Tree query is ", Average_rtree_time)
    
    
    r =0 
    Seq_query_start=time.time() #start time to scan query using sequential
    for p in points:
        for q in queries:
                if q['x1'] <= p['x'] <= q['x2'] and q['y1'] <= p['y'] <= q['y2']: #Check if the point is between others 2 points 
                    r = r+1 #check the next point
    #Display query time in sequential scan benchmark
    Seq_query_end=time.time() #end time to sequential scan
    Seq_time=Seq_query_end-Seq_query_start #query processing time
    Average_seq_time = Seq_time/100 #average time run each query using sequential 
    print ("The sequential query time is",Seq_time,"seconds.")
    print("The average time of sequential query is ", Average_seq_time)

    #Checking the faster between R-Tree and Sequential scan
    fas = Average_seq_time / Average_rtree_time
    print("R-Tree is", fas, " times faster than sequential query.")
